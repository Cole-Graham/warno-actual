...
from src import ndf
from src.dics import rda_unit_edits, sov_unit_edits, usa_unit_edits
from src.utils.logging_utils import setup_logger
from src.utils.ndf_utils import find_namespace

# Set up logger
logger = setup_logger('unit_edits')

# Get config instance
config = ConfigLoader("config/config.YAML").load()

# Load unit edits at module level
units = {}
units.update(rda_unit_edits)
units.update(usa_unit_edits)
units.update(sov_unit_edits)

def load_unit_edits() -> Dict:
    """Load and merge all unit edit dictionaries."""
    merged_edits = {}
    dics_path = Path(__file__).parent / "dics"
    ...
    for file in dics_path.glob("*unit_edits.py"):
        ...
        try:
            ...
            # Look for nation-specific dictionary names
            dict_name = f"{file.stem.split('_')[0]}_unit_edits"
            if hasattr(module, dict_name):
                merged_edits.update(getattr(module, dict_name))
                logger.info(f"Loaded unit edits from {file.stem}")
            else:
                logger.warning(f"No '{dict_name}' dictionary found in {file.stem}")
...
def process_unitdescriptor(source):
    """Main entry point for unit descriptor processing."""
    modify_unit_descriptors(source)


def modify_unit_descriptors(source: Any) -> None:
    """Modify unit descriptors in UniteDescriptor.ndf."""
    ...
    for unit_row in source:
        ...
        edits = find_namespace(unit_row, "Descriptor_Unit_", units)
        if edits is None:
            continue
...
def process_unitinfantry(source):
    """Process infantry-specific edits."""
