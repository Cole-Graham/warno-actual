...
def copy_fx_files(config: Dict) -> None:
    """Copy FX files from src/constants/fx/generated to mod directory.
    
    Args:
        config: Configuration dictionary containing paths
    """
    source_dir = Path("src/constants/fx/generated")
    if not source_dir.exists():
        logger.warning(f"FX generated directory not found: {source_dir}")
        return
    ...
    # Only copy FX files for gameplay builds
    ...
    # Target is GameData/Fx/Generated/
    target_dir = warno_mods / target_mod / config['asset_config']['target_dir'] / "Fx" / "Generated"
    
    with log_time(logger, "Copying FX files"):
        # Walk through all files in FX generated directory
        for source_file in source_dir.rglob('*'):
            # Only copy .ndf files
            if not source_file.is_file() or source_file.suffix.lower() != '.ndf':
                continue
                
            # Get relative path from generated root
            ...
            try:
                shutil.copy2(source_file, target_path)
                logger.debug(f"Copied FX {rel_path} to {target_path}")
            except Exception as e:
                logger.error(f"Failed to copy FX {rel_path}: {e}")