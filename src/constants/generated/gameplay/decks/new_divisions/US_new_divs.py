"""New divisions for USA."""

from .new_divisionrules import (
    US_airborne_newdivisionrules,
    US_armored_newdivisionrules,
    US_global_newdivisionrules,
    US_marine_newdivisionrules,
    US_mechanized_newdivisionrules,
    US_motorized_newdivisionrules,
)

us_new_divs = {
    "US_general": {
        "division_id": 5001,
        "interface_order": 1000,
        "guid": "0dc2088a-9fca-4925-97c2-166cf79642fd",
        "cfg_name": "US_national_general",
        "div_name": ("USA", "BUKHWKTHUH"),
        "description_title": ("USA", "ZWWJULIVOW"),
        "summary_text": ("USA combined arms division.", "SUMUSGEN"),
        "history_text": ("USA national division.", "HISUSGEN"),
        "activation_points": 75,
        "standout_units": ["M1A1HA_Abrams_US", "GreenBerets_ODA_US", "F117_Nighthawk_US"],
        "division_rules": [
            US_airborne_newdivisionrules,
            US_armored_newdivisionrules,
            US_global_newdivisionrules,
            US_marine_newdivisionrules,
            US_mechanized_newdivisionrules,
            US_motorized_newdivisionrules,
        ],
        "rule_exclusions": [
            "Gama_Goat_supply_US",
            "M60A1_RISE_Passive_CMD2_US",
            "HMGteam_Mk19_USMC_US",
            "HMGteam_Mk19_US",
            "HMGteam_M2HB_US",
            "HMGteam_M60_US",
            "81mm_mortar_Aero_US",
            "Mortier_107mm_US",
            "Howz_M198_155mm_US",
            "M109A2_HOWZ_US",
            "M110A2_HOWZ_US",
            "M1025_Humvee_TOW_US",
            "M60A1_RISE_Passive_CMD_US",
            "MANPAD_Stinger_C_US",
            "MANPAD_Stinger_C_USMC_US",
            "DCA_I_Hawk_US",
            "M998_Avenger_nonPara_US",
            "DCA_M167A2_Vulcan_20mm_US",
        ],
        # "matrix_overrides": {
        #     "Factory/Logistic": [2, 2, 2, 2, 3, 3, 3, 3, 3],
        # },
        "transport_overrides": {
            "MP_US": ["M1038_Humvee_US"],
            "MP_RCL_US": ["CUCV_MP_US", "CUCV_HMG_US", "CUCV_AGL_US", "M1025_Humvee_MP_US"],
            "Rifles_half_CMD2_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US", "UH1N_TwinHuey_RKT_US", "CH47_Chinook_US"],
            "Rifles_half_CMD_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US","M113A3_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_LAW_US": ["M998_Humvee_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_AT4_US": ["M998_Humvee_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_Dragon_US": ["M998_Humvee_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
        },
    },
    "US_airborne": {
        "division_id": 5002,
        "interface_order": 1001,
        "guid": "a5729c4e-a163-4dd7-8e99-a39fb9ca5d31",
        "cfg_name": "US_national_airborne",
        "div_name": ("USA Airborne", "NRKJFPTFWT"),
        "description_title": ("USA Airborne", "FVETLAABRD"),
        "summary_text": ("USA airborne division.", "SUMUSAIR"),
        "history_text": ("USA Airborne division.", "HISUSAIR"),
        "activation_points": 100,
        "standout_units": ["DeltaForce_US", "AH64_Apache_ATAS_US", "F15E_StrikeEagle_US"],
        "division_rules": [
            US_airborne_newdivisionrules,
            US_global_newdivisionrules,
        ],
    },
    "US_airborne_armored": {
        "division_id": 5003,
        "interface_order": 1002,
        "guid": "e4107dc6-2618-4579-b2f6-dd8f6f160e8d",
        "cfg_name": "US_national_airborne_armored",
        "div_name": ("USA Airborne / Armored", "JRXFQANUWW"),
        "description_title": ("USA Airborne / Armored", "BPAZWZQOQP"),
        "summary_text": ("USA airborne and armored division.", "SUMUSAARM"),
        "history_text": ("USA Airborne / Armored division.", "HISUSAARM"),
        "activation_points": 85,
        "standout_units": ["M1A1HA_Abrams_US", "AeroRifles_AT4_US", "F15C_Eagle_AA_US"],
        "division_rules": [
            US_airborne_newdivisionrules,
            US_armored_newdivisionrules,
            US_global_newdivisionrules,
        ],
        "rule_exclusions": [
            "HMGteam_Mk19_US",
            "HMGteam_M2HB_US",
            "HMGteam_M60_US",
            "M998_Avenger_nonPara_US",
            "DCA_M167A2_Vulcan_20mm_US"
        ],
        "transport_overrides": {
            "Rifles_half_CMD2_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M113_Dragon_US", "M2A1_Bradley_IFV_US", "UH60A_Black_Hawk_US", "CH47_Chinook_US"],
        },
    },
    "US_airborne_marine": {
        "division_id": 5004,
        "interface_order": 1003,
        "guid": "423b9035-c6da-4bf7-af5e-0c6dcf3f9ac9",
        "cfg_name": "US_national_airborne_marine",
        "div_name": ("USA Airborne / Marine", "FMGRGALOLY"),
        "description_title": ("USA Airborne / Marine", "PFMOPXITOW"),
        "summary_text": ("USA airborne and marine division.", "SFQQRMONEK"),
        "history_text": ("USA Airborne / Marine division.", "MHNXKTJJJR"),
        "activation_points": 85,
        "standout_units": ["Airborne_US", "Navy_SEAL_US", "F14A_Tomcat_AA2_US"],
        "division_rules": [
            US_airborne_newdivisionrules,
            US_global_newdivisionrules,
            US_marine_newdivisionrules,
        ],
        "rule_exclusions": [
            "HMGteam_Mk19_USMC_US",
            "ATteam_TOW2_USMC_US",
            "MANPAD_Stinger_C_US",
        ],
        "transport_overrides": {
            "Rifles_half_CMD2_US": ["M998_Humvee_HMG_US", "AAVP_7A1_USMC_US", "UH1N_TwinHuey_US", "UH1N_TwinHuey_RKT_US", "CH47_Chinook_US"],
        },
    },
    "US_airborne_mechanized": {
        "division_id": 5005,
        "interface_order": 1004,
        "guid": "a32d7229-4dcc-4673-855b-3e453fee1846",
        "cfg_name": "US_national_airborne_mechanized",
        "div_name": ("USA Airborne / Mechanized", "QKMSZBHSHW"),
        "description_title": ("USA Airborne / Mechanized", "QEVEXLTTGG"),
        "summary_text": ("USA airborne and mechanized division.", "SUMUSAMEC"),
        "history_text": ("USA Airborne / Mechanized division.", "HISUSAMEC"),
        "activation_points": 85,
        "standout_units": ["Airborne_US", "M2A1_Bradley_IFV_US", "F4_Wild_Weasel_2_US"],
        "division_rules": [
            US_airborne_newdivisionrules,
            US_global_newdivisionrules,
            US_mechanized_newdivisionrules,
        ],
        "rule_exclusions": [
            "HMGteam_Mk19_US",
            "HMGteam_M2HB_US",
            "HMGteam_M60_US",
            "M998_Avenger_nonPara_US",
            "DCA_M167A2_Vulcan_20mm_US",
        ],
        "transport_overrides": {
            "Rifles_half_CMD2_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US", "UH60A_Black_Hawk_US", "CH47_Chinook_US"],
            "Sniper_US": ["M998_Humvee_US", "M1025_Humvee_scout_US", "M1025_Humvee_AGL_nonPara_US"],
            "Sniper_M82_US": ["M998_Humvee_US", "M1025_Humvee_scout_US", "M1025_Humvee_AGL_nonPara_US"],
        },
    },
    "US_airborne_motorized": {
        "division_id": 5006,
        "interface_order": 1005,
        "guid": "d8a90f57-eec1-4399-a9c9-74c8d83b5fff",
        "cfg_name": "US_national_airborne_motorized",
        "div_name": ("USA Airborne / Motorized", "LEOKKLGOZQ"),
        "description_title": ("USA Airborne / Motorized", "BEDOJYYUIU"),
        "summary_text": ("USA airborne and motorized division.", "SUMUSAMOT"),
        "history_text": ("USA Airborne / Motorized division.", "HISUSAMOT"),
        "activation_points": 85,
        "standout_units": ["LightRifles_AT4_US", "LAV_25_M1047_US_US", "FA16_CAS_US"],
        "division_rules": [
            US_airborne_newdivisionrules,
            US_global_newdivisionrules,
            US_motorized_newdivisionrules,
        ],
        "rule_exclusions": [
            "HMGteam_Mk19_US",
            "HMGteam_M2HB_US",
            "HMGteam_M60_US",
            "M998_Avenger_nonPara_US",
            "M1025_Humvee_TOW_US",
            "DCA_M167A2_Vulcan_20mm_US",
        ],
        "transport_overrides": {
            "Rifles_half_CMD2_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "UH60A_Black_Hawk_US", "CH47_Chinook_US"],
            "MP_US": ["M1038_Humvee_US"],
            "MP_RCL_US": ["CUCV_MP_US", "CUCV_HMG_US", "CUCV_AGL_US", "M1025_Humvee_MP_US"],
            "MP_Patrol_USAF_US": ["M998_Humvee_US", "CUCV_MP_US", "CGage_Peacekeeper_US", "CUCV_HMG_US", "CUCV_AGL_US"],
            "LRRP_US": ["M998_Humvee_US", "M1025_Humvee_scout_US", "M1025_Humvee_AGL_nonPara_US", "UH60A_Black_Hawk_US"],
            "Sniper_US": ["M998_Humvee_US", "M1025_Humvee_scout_US", "M1025_Humvee_AGL_nonPara_US"],
            "Sniper_M82_US": ["M998_Humvee_US", "FAV_trans_US","M1025_Humvee_scout_US", "M1025_Humvee_AGL_nonPara_US"],
        },
    },
    "US_armored": {
        "division_id": 5007,
        "interface_order": 1006,
        "guid": "fbe63b07-12ba-42a2-83d7-256da9eaee93",
        "cfg_name": "US_national_armored",
        "div_name": ("USA Armored", "QFULKVAART"),
        "description_title": ("USA Armored", "FDEHSHQXMV"),
        "summary_text": ("USA armored division.", "SUMUSARM"),
        "history_text": ("USA Armored division.", "HISUSARM"),
        "activation_points": 100,
        "standout_units": ["M1A1HA_Abrams_CMD_US", "M3A2_Bradley_CFV_US", "AH64_Apache_US"],
        "division_rules": [
            US_armored_newdivisionrules,
            US_global_newdivisionrules,
        ],
    },
    "US_armored_marine": {
        "division_id": 5008,
        "interface_order": 1007,
        "guid": "ebd90eb4-35bd-4c12-9c99-037be15b3a8f",
        "cfg_name": "US_national_armored_marine",
        "div_name": ("USA Armored / Marine", "IXIIQMSLWB"),
        "description_title": ("USA Armored / Marine", "JORUIVYORJ"),
        "summary_text": ("USA armored and marine division.", "XKWVDSXOPO"),
        "history_text": ("USA Armored / Marine division.", "KCHFBUOXGG"),
        "activation_points": 85,
        "standout_units": ["M1A1HA_Abrams_US", "Navy_SEAL_US", "F14A_Tomcat_AA2_US"],
        "division_rules": [
            US_armored_newdivisionrules,
            US_global_newdivisionrules,
            US_marine_newdivisionrules,
        ],
        "rule_exclusions": [
            "M1025_Humvee_CMD_US",
            "M60A1_RISE_Passive_CMD2_US",
            "HMGteam_Mk19_US",
            "HMGteam_M2HB_US",
            "ATteam_TOW2_US",
            "M109A2_HOWZ_US",
            "M60A1_RISE_Passive_CMD_US",
            "MANPAD_Stinger_C_US",
            "DCA_I_Hawk_US",
        ],
        "transport_overrides": {},
    },
    "US_armored_mechanized": {
        "division_id": 5009,
        "interface_order": 1008,
        "guid": "13d29ac2-28ce-481f-8e06-c1849f7fcd36",
        "cfg_name": "US_national_armored_mechanized",
        "div_name": ("USA Armored / Mechanized", "DNMHIFVPBS"),
        "description_title": ("USA Armored / Mechanized", "JZIMHUFNZD"),
        "summary_text": ("USA armored and mechanized division.", "SUMUSARMC"),
        "history_text": ("USA Armored / Mechanized division.", "HISUSARMC"),
        "activation_points": 85,
        "standout_units": ["M1A1_Abrams_CMD_US", "M2A1_Bradley_IFV_US", "F111F_Aardvark_LGB_US"],
        "division_rules": [
            US_armored_newdivisionrules,
            US_global_newdivisionrules,
            US_mechanized_newdivisionrules,
        ],
        "transport_overrides": {
            "Rifles_half_CMD_US": ["M998_Humvee_US", "M113A3_US", "M113_Dragon_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_LAW_US": ["M998_Humvee_US", "M113A3_US", "M113_Dragon_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_AT4_US": ["M998_Humvee_US", "M113A3_US", "M113_Dragon_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_Dragon_US": ["M998_Humvee_US", "M113A3_US", "M113_Dragon_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
        },
    },
    "US_armored_motorized": {
        "division_id": 5010,
        "interface_order": 1009,
        "guid": "a6ae807e-1e3a-44c9-a4e4-a222b240bc91",
        "cfg_name": "US_national_armored_motorized",
        "div_name": ("USA Armored / Motorized", "SWZWKORUGN"),
        "description_title": ("USA Armored / Motorized", "JTIJZHFWBO"),
        "summary_text": ("USA armored and motorized division.", "SUMUSARMT"),
        "history_text": ("USA Armored / Motorized division.", "HISUSARMT"),
        "activation_points": 85,
        "standout_units": ["M1A1_Abrams_US", "LightRifles_AT4_US", "AH64_Apache_emp2_US"],
        "division_rules": [
            US_armored_newdivisionrules,
            US_global_newdivisionrules,
            US_motorized_newdivisionrules,
        ],
        "transport_overrides": {
            "MP_US": ["M1038_Humvee_US"],
            "MP_RCL_US": ["CUCV_MP_US", "CUCV_HMG_US", "CUCV_AGL_US", "M1025_Humvee_MP_US"],
            "Rifles_half_CMD2_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M2A1_Bradley_IFV_US", "UH60A_Black_Hawk_US", "CH47_Chinook_US"],
            "Rifles_half_CMD_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US","M113A3_US", "M2A1_Bradley_IFV_US"],
            "Rifles_half_LAW_US": ["M998_Humvee_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US"],
            "Rifles_half_AT4_US": ["M998_Humvee_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US"],
            "Rifles_half_Dragon_US": ["M998_Humvee_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US"],
            "ATteam_TOW2A_US": ["M151_MUTT_trans_US", "CUCV_HMG_US", "CUCV_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
            "ATteam_TOW2_US": ["M151_MUTT_trans_US", "CUCV_HMG_US", "CUCV_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
            "ATteam_ITOW_US": ["M151_MUTT_trans_US", "CUCV_HMG_US", "CUCV_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
            "Rangers_CMD_US": ["M1038_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
            "Ranger_US": ["M1038_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
            "Ranger_Dragon_US": ["M1038_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
        },
    },
    "US_marine": {
        "division_id": 5030,
        "interface_order": 1010,
        "guid": "45edf1a3-f875-4965-9c2e-12f87baaa6e1",
        "cfg_name": "US_national_marine",
        "div_name": ("USA Marine", "BDGRNKYODG"),
        "description_title": ("USA Marine", "FDOIXUVXQG"),
        "summary_text": ("USA Marine division.", "MGNCPPCRAN"),
        "history_text": ("USA Marine division.", "MYIJWFTSDD"),
        "activation_points": 100,
        "standout_units": ["Groupe_AT_USMC_US", "Navy_SEAL_US", "CATFAE_US", "UH1N_TwinHuey_RKT_US", "F18_Hornet_SEAD_US", "F14A_Tomcat_AA2_US"],
        "division_rules": [
            US_marine_newdivisionrules,
            US_global_newdivisionrules,
        ],
    },
    "US_marine_mechanized": {
        "division_id": 5011,
        "interface_order": 1011,
        "guid": "2b87bece-da79-45e4-ba02-774b89f2f6f4",
        "cfg_name": "US_national_marine_mechanized",
        "div_name": ("USA Marine / Mechanized", "EVZOAAWXAZ"),
        "description_title": ("USA Marine / Mechanized", "OXRQXFDJZS"),
        "summary_text": ("USA marine and mechanized division.", "FTBPOZFUME"),
        "history_text": ("USA Marine / Mechanized division.", "BTFBHXCGFY"),
        "activation_points": 85,
        "standout_units": ["Rifles_half_Cav_CMD_US", "M2A2_BRAT_US", "F14A_Tomcat_AA2_US"],
        "division_rules": [
            US_marine_newdivisionrules,
            US_global_newdivisionrules,
            US_mechanized_newdivisionrules,
        ],
        "rule_exclusions": [
            "M1025_Humvee_CMD_US",
            "M60A1_RISE_Passive_CMD2_US",
            "HMGteam_M2HB_US",
            "HMGteam_Mk19_US",
            "ATteam_TOW2_US",
            "M110A2_HOWZ_US",
            "M60A1_RISE_Passive_CMD_US",
            "MANPAD_Stinger_C_US",
            "DCA_I_Hawk_US",
        ],
        "transport_overrides": {
            "Rifles_half_CMD2_US": ["M998_Humvee_HMG_US", "AAVP_7A1_USMC_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US", "UH1N_TwinHuey_RKT_US", "CH47_Chinook_US"],
        },
    },
    "US_marine_motorized": {
        "division_id": 5012,
        "interface_order": 1012,
        "guid": "dd683568-227b-4f14-8a66-c604487d08c8",
        "cfg_name": "US_national_marine_motorized",
        "div_name": ("USA Marine / Motorized", "QCTQWCCRLZ"),
        "description_title": ("USA Marine / Motorized", "MSZDABQMPA"),
        "summary_text": ("USA marine and motorized division.", "ZRMTDDSGCC"),
        "history_text": ("USA Marine / Motorized division.", "BQFFOTDNFF"),
        "activation_points": 85,
        "standout_units": ["DeltaForce_US", "CUCV_Hellfire_US", "FA16_CAS_US", "F14A_Tomcat_AA2_US"],
        "division_rules": [
            US_marine_newdivisionrules,
            US_global_newdivisionrules,
            US_motorized_newdivisionrules,
        ],
        "rule_exclusions": [
            "M1025_Humvee_CMD_US",
            "M60A1_RISE_Passive_CMD2_US",
            "HMGteam_M2HB_US",
            "HMGteam_Mk19_US",
            "ATteam_TOW2_US",
            "Howz_M198_155mm_US",
            "M109A2_HOWZ_US",
            "M60A1_RISE_Passive_CMD_US",
            "MANPAD_Stinger_C_US",
        ],
        "transport_overrides": {
            "Rifles_half_CMD2_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "AAVP_7A1_USMC_US", "UH1N_TwinHuey_RKT_US", "CH47_Chinook_US"],
        },
    },
    "US_mechanized": {
        "division_id": 5014,
        "interface_order": 1013,
        "guid": "1be5bd50-92da-4c66-aa1e-ef1b6eebb0be",
        "cfg_name": "US_national_mechanized",
        "div_name": ("USA Mechanized", "TWCAJWWYYT"),
        "description_title": ("USA Mechanized", "XGPPTRINYD"),
        "summary_text": ("USA mechanized division.", "SUMUSMEC"),
        "history_text": ("USA Mechanized division.", "HISUSMEC"),
        "activation_points": 100,
        "standout_units": ["M1IP_Abrams_US", "NatGuard_M67_US", "F111F_Aardvark_LGB_US"],
        "division_rules": [
            US_global_newdivisionrules,
            US_mechanized_newdivisionrules,
        ],
        # "matrix_overrides": {
        #     "Factory/Logistic": [2, 2, 2, 2, 3, 3, 3, 3, 3],
        # },
    },
    "US_mechanized_motorized": {
        "division_id": 5015,
        "interface_order": 1014,
        "guid": "bfb01779-4c5a-4188-88ad-e15f618d71ad",
        "cfg_name": "US_national_mechanized_motorized",
        "div_name": ("USA Mechanized / Motorized", "ADMMJBOYSE"),
        "description_title": ("USA Mechanized / Motorized", "RQTJYSELLW"),
        "summary_text": ("USA mechanized and motorized division.", "SUMUSMECT"),
        "history_text": ("USA Mechanized / Motorized division.", "HISUSMECT"),
        "activation_points": 85,
        "standout_units": ["M1IP_Abrams_US", "Sniper_M82_US", "F4_Wild_Weasel_2_US"],
        "division_rules": [
            US_global_newdivisionrules,
            US_mechanized_newdivisionrules,
            US_motorized_newdivisionrules,
        ],
        # "matrix_overrides": {
        #     "Factory/Logistic": [2, 2, 2, 2, 3, 3, 3, 3, 3],
        # },
        "transport_overrides": {
            "MP_US": ["M1038_Humvee_US"],
            "MP_RCL_US": ["CUCV_MP_US", "CUCV_HMG_US", "CUCV_AGL_US", "M1025_Humvee_MP_US"],
            "Rifles_half_CMD2_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US", "UH60A_Black_Hawk_US", "CH47_Chinook_US"],
            "Rifles_half_CMD_US": ["M998_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "M113A3_US", "M2A2_Bradley_IFV_US", "UH1H_Huey_US"],
            "Rifles_half_LAW_US": ["CUCV_trans_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_AT4_US": ["CUCV_trans_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rifles_half_Dragon_US": ["CUCV_trans_US", "CUCV_HMG_US", "CUCV_AGL_US", "M113A3_US", "M2A1_Bradley_IFV_US", "M2A2_Bradley_IFV_US"],
            "Rangers_CMD_US": ["M1038_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
            "Ranger_US": ["M1038_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
            "Ranger_Dragon_US": ["M1038_Humvee_US", "M998_Humvee_HMG_US", "M998_Humvee_AGL_US", "UH1H_Huey_US", "UH60A_Black_Hawk_US"],
        },
    },
    "US_motorized": {
        "division_id": 5016,
        "interface_order": 1015,
        "guid": "9814c02d-4a96-4515-acba-98f2720579ca",
        "cfg_name": "US_national_motorized",
        "div_name": ("USA Motorized", "GLVOWMTEIW"),
        "description_title": ("USA Motorized", "GWDWLGIIRN"),
        "summary_text": ("USA motorized division.", "SUMUSMOT"),
        "history_text": ("USA Motorized division.", "HISUSMOT"),
        "activation_points": 100,
        "standout_units": ["M1_Abrams_US", "LightRifles_Dragon_US", "FA16_CAS_US", "F117_Nighthawk_US"],
        "division_rules": [
            US_motorized_newdivisionrules,
            US_global_newdivisionrules,
        ],
        # "matrix_overrides": {
        #     "Factory/Logistic": [2, 2, 2, 2, 2, 3, 3, 3, 3],
        # },
    },
}