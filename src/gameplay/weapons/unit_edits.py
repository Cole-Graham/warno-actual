...
def _gather_turret_templates(source_path: Any, unit: str, edits: Dict, ammo_db: Dict[str, Any],
                             weapon_db: Dict[str, Any]) -> List[Tuple[str, Any]]:
...
def _find_turret_templates(weapon_descr: Any, add_list: List, weapons_to_add: List, weapon_descr_data: Dict,
                           weapon_db: Dict, ammo_db: Dict, source_path: Any) -> List[Tuple[str, Any]]:
...
def _apply_salvo_changes(weapon_descr: Any, wd_edits: Dict, weapon_descr_data: Dict, game_db: Dict) -> None:
        ...
        if weapon in salvo_mapping:
            ...
            if "replace" in wd_edits["equipmentchanges"]:
                for old_weapon, new_weapon in wd_edits["equipmentchanges"]["replace"]:
                    ...
                    index = salvo_mapping[old_weapon_old_name if old_weapon_old_name else old_weapon]
    ...
    if "add" in salve_edits:
        for addition in salve_edits["add"]:
...
def _adjust_light_at_salvos(weapon_descr: Any, unit_name: str, ammos_: Dict, ammo_db: Dict,
                            unit_db: Dict, weapon_db: Dict) -> None:
