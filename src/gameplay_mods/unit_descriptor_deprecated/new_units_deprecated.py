...
#             # Handle WeaponDescriptor
#             if descr_row.v.startswith("$/GFX/Weapon/WeaponDescriptor_"):
#                 if not edits.get("is_unarmed", False):
#                     modules_list.v.replace(descr_row, f"$/GFX/Weapon/WeaponDescriptor_{edits['NewName']}")
#                 else: 
#                     modules_list.v.remove(descr_row)
...
#             elif descr_type == "TGenericMovementModuleDescriptor":
#                 if "max_speed" in edits:
#                     descr_row.v.by_member("MaxSpeedInKmph").v = str(edits["max_speed"])  # noqa
                    
#             elif descr_type == "TLandMovementModuleDescriptor":
#                 pass
            
#             elif descr_type == "AirplaneMovementDescriptor":
#                 if "max_speed" in edits:
#                     descr_row.v.by_member("SpeedInKmph").v = str(edits["max_speed"])  # noqa
                
#             elif descr_type == "TInfantrySquadModuleDescriptor":
#                 _handle_infantry_squad(descr_row, edits)
...
#                     descr_row.v.by_member("ProductionRessourcesNeeded").v.by_key(cmd_points).v = str(
#                         edits["CommandPoints"]
#                     )  # noqa
...
#                 f")"
#             )
#             production_module_index = modules_list.find_by_cond(
...
#             logger.info(f"(modify_new_unit) Added capacity module to {unit_row.namespace}")
...
# def _handle_infantry_squad(descr_row: Any, edits: Dict[str, Any]) -> None:
#     """Handle infantry squad module modifications."""
#     mimetic_descr = descr_row.v.by_member("MimeticDescriptor")
#     mimetic_descr.v.by_member("DescriptorId").v = f"GUID:{{{edits['GroupeCombatGUID']}}}"

#     if "strength" in edits:
#         descr_row.v.by_member("NbSoldatInGroupeCombat").v = str(edits["strength"])

#     if "NewName" in edits:
#         descr_row.v.by_member("InfantryMimeticName").v = f"'{edits['NewName']}'"
#         descr_row.v.by_member("WeaponUnitFXKey").v = f"'{edits['NewName']}'"
...
# def _handle_capacities_module(default_skill_list: Any, edits: Dict[str, Any]) -> None:
