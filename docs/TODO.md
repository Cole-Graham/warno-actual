 checkboxes for completed items: - [x], - [ ]
 
 Lat:
- [ ] Recon helos are insanely cheap in wargame terms, not sure if they should be,
depends on performance of planes hunting helos. Might turn off range scaling for plane
guns.
- [ ] Regenerate maps/scenarios (Eugen broke them again)
- [ ] Validate that when laser guided bombs miss, they miss by a lolt
- [ ] Make room for thicker division list scroll bars in armory
- [ ] Change SPW PSH security transport to Normal optics
- [ ] Create new "High Skill Unit" trait for Wild Weasel 2, because its funny and to explain that the
SEAD missiles have reflex fire on even when you right click a target, in order to enable the SEAD missiles 
to still fire when you right click a tank to fire the Maverick missiles on.
- [ ] Might need seperate weaker version of EW trait for Pave Tiger drone
- [ ] Consolidate 4-5 strength police infantry into bigger mixed squads of MPs + 2-3 common reservists with AT
- [ ] Fix M60E1 MMG ammo on reservists
- [ ] Rescale splash radius/adjust dispersion for cluster bombs to hopefully fix low damage of area bombers from radius damage falloff
- [ ] See if it is possible to remove smart order from auto-cover toggle
- [ ] LDR infantry should probably be lowered to same price as equivalent infantry, or 5 points higher
- [ ] Rebalance air prices around 350 point ceiling (maybe introduce second more powerful strike eagle variant)
- [ ] change supply traits to be more coherent with gameplay use-cases
- [ ] create custom model with more turrets for Su-24M bomber
- [ ] remove submunition release animation from MW1 and KMGU bombers
- [ ] Give ASFs unique stress effect without accuracy modifier, 
or just remove stress accuracy modifier from planes entirely
- [ ] Edit Buratino model to add turret for smoke launcher (then revert front armor buff after, probably)


12:
- [ ] Add non-USSR/US LANDJUT units
- [ ] Fix G3 Geb sniper
- [ ] US/USSR/POL marine

IDK who: 
- [ ] Rework Black Tulip?